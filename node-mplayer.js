...
Mplayer.prototype.play = function(opts) {
  if(this.file !== null){
    ...
    //TODO investigate error with -volume 100 (startup error 1)
    var args = ['-slave', '-idle','-quiet', '-novideo', this.file],
    that = this;

    console.log("spawn mplayer "+args.join(" ") );
    ...
    this.childProc.on('error', function(error){
      that.emit('error','childProcess error');
    });
    ...
    this.childProc.on('exit', function(code, sig){
      ...
      if(code === 0 && sig === null){
        ...
        that.emit('error','mplayer returned code '+code);
      }
    });
...
Mplayer.prototype.setVolume = function(volume) {
  if(this.childProc !== null){
    ...
    this.childProc.stdin.write('volume ' + this.volume + ' 1\n');
  }
};
